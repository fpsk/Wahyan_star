...
def process_year_pdf(pdf_path, year, output_base_dir):
    ...
    for idx in range(total_pages):
        ...
        if not os.path.exists(full_img_path):
            ...
            if page_images:
                try:
                    ...
                    print(f"Year {year} Page {page_num} image extraction warning: {e}")
        ...
        if page_num % 15 == 0 or page_num == total_pages:
            print(f"Finished Year {year} Page {page_num} / {total_pages}...", flush=True)

    return year_manifest

def process_all_files(files_dir, output_base_dir):
    pdf_files = [f for f in os.listdir(files_dir) if f.endswith('.pdf') and not f.endswith('OCR.pdf')]
    pdf_files.sort()

    all_years_data = {}
    master_index = []

    print(f"Found {len(pdf_files)} Wah Yan Star PDF volume(s) in '{files_dir}': {pdf_files}")

    for pdf_name in pdf_files:
        # Extract year from filename (e.g. 1976.pdf -> 1976)
        match = re.search(r'\b(19\d\d|20\d\d)\b', pdf_name)
        year = int(match.group(1)) if match else 1976

        pdf_path = os.path.join(files_dir, pdf_name)
        year_pages = process_year_pdf(pdf_path, year, output_base_dir)
        all_years_data[year] = year_pages

        # Build search index entries
        for p in year_pages:
            md_file = os.path.join(output_base_dir, p["file"])
            text = ""
            ...
            master_index.append({
                "year": year,
                "page": p["page"],
                "title": f"Wah Yan Star {year} - Page {p['page']}",
                "text": text,
                "photos": p["photos"],
                "full_page_photo": f"{year}_page_{p['page']:03d}_full.png"
            })

    # Save Master Search Index
    search_json_path = "okf_search_data.json"
    with open(search_json_path, "w", encoding="utf-8") as f:
        json.dump(master_index, f, ensure_ascii=False, indent=2)

    print(f"\n==========================================")
    print(f" Master Multi-Year Dataset Built Successfully!")
    print(f" Total Volumes Processed: {len(pdf_files)}")
    print(f" Total Master Search Entries: {len(master_index)}")
    print(f" Master Index Saved: {search_json_path}")
    print(f"==========================================")

if __name__ == "__main__":
    files_directory = "Files"
    okf_directory = "okf_output"
    process_all_files(files_directory, okf_directory)
